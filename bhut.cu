...
*/
void barnes_hut_cuda(std::vector<float4> &bodys, std::vector<float3> &velocitys, int total_iters, float dt){
    //////////// START DATA INIT ////////////
    ...
    compute_forces_kernel<<<grid_dim, block_dim>>>(d_x, d_y, d_z, d_mass, d_children, N, max_nodes, root_half, d_Fx, d_Fy, d_Fz);
...
/* write updated points and computed force on them */
void write_output(std::ofstream& file, float *x, float *y, float *z, 
...
int main(){
    ...
    while (file >> x >> y >> z >> mass){
        ...
        velo.push_back(make_float3(0.0f,0.0f,0.0f));
    }
    std::cout << "N=" << bodys.size() << "\n";

    barnes_hut_cuda(bodys, velo, total_iters, dt);
}